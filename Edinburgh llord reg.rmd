...
# df_foi_dz has 601 rows but there are only 597 DZ in Edin
# one is na and there are four which appear to be outside Edinburgh
...
```
To aggregate unit postcodes to Datazones, we use a lookup file. In this process, the lookup failed in just `r sum(is.na(df_foi$pc7))` cases - a very small fraction. These cases contained `r sum(df_foi$NumberOfProperties[is.na(df_foi$DataZone2011)])` properties. 

There are `r nrow(df_dz_Edin)` Datazones in Edinburgh in 2011. 

There are `r sum(df_foi_dz$NumberOfProperties[is.na(df_foi_dz$DataZone2011)])` cases on the FOI dataset where the Datazone is missing. 


In the FOI data, there appear and `r nrow(df_foi_dz)` of these have registered landlords in 2017. 
...
#### Figure 4: Households in PRS - 2011 census
![](maps/Census_pct.png) 
<br>
